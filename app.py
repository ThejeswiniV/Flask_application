...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:keerthi123@localhost/gwcpmplocal"
engine = create_engine('mysql+pymysql://root:keerthi123@localhost/gwcpmplocal')
db = SQLAlchemy(app)
...
class Subtask(db.Model):
    ...
    SubtaskType = db.Column(db.String(50), default='Task', nullable=False,)
...
def add_subtask(story_id):
    ...
    if request.method == 'POST':
        ...
        subtask_type = request.form['subtask_type']  # Get subtask type from the form
        ...
        subtask = Subtask(
            ...
            StoryID=story_id,
            SubtaskType=subtask_type,  # Save subtask type to the database
            Priority=priority  # Save priority to the database
        )
...
def update_subtask(subtask_id):
    ...
    if request.method == 'POST':
        ...
        subtask.SubtaskType = request.form.get('SubtaskType')  # Get subtask type from the form
        subtask.Priority = request.form.get('Priority')  # Get priority from the form
        db.session.commit()
